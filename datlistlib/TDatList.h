...
{
	DatLink<ValType>* pTemp = dl.pFirst;
	pFirst = new TDatLink<ValType>(pTemp->Value, NULL);
	TDatLink<ValType>* TmpLast = pFirst;
	TDatLink<ValType>* TmpCurr;
	for (int i = 1; i < dl.listLen; i++)
	{
		pTemp = pTemp->GetNextLink();
		TmpCurr = new TDatLink<ValType>(pTemp->Value, NULL);
